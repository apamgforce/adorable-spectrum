...
const CONTACT_IMG = "https://images.unsplash.com/photo-1531206715517-5c0ba140b2b8?w=900&q=85&auto=format&fit=crop";
    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          name: form.name,
          email: form.email,
          subject: `GFA Form Submission: ${form.subject || "General Inquiry"}`,
      ...
      <section className="pt-32 pb-16 px-6" style={{ background: 'var(--cream)' }}>
        <div className="max-w-7xl mx-auto">
          <div className="reveal max-w-xl">
            <span className="text-xs tracking-[0.25em] uppercase font-medium" style={{ color: 'var(--sage)' }}>Reach Out</span>
            <h1 className="font-display text-5xl lg:text-6xl font-light mt-2 leading-tight" style={{ color: 'var(--forest)' }}>
              Let&apos;s plant<br />
              <em style={{ color: 'var(--sage)' }}>something together.</em>
            </h1>
          </div>
        </div>
      </section>
          ...
          <div className="reveal">
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-4 items-start p-5 rounded-2xl" style={{ background: 'var(--mist)' }}>
                ...
                <div>
                  <p className="font-medium text-sm" style={{ color: 'var(--forest)' }}>Phone</p>
                  <p className="text-slate-500 text-sm mt-1">+233 (0) 24 571 8457 / +233 (0) 50 094 0262</p>
                </div>
              </div>
            </div>
            ...
            <div className="scripture mt-8 px-6 py-5 rounded-2xl">
              <p className="font-display text-lg italic" style={{ color: 'var(--forest)' }}>
                &ldquo;Two are better than one... if either of them falls down, one can help the other up.&rdquo;
              </p>
              <span className="text-sm mt-2 block" style={{ color: 'var(--gold)' }}>— Ecclesiastes 4:9–10</span>
            </div>
          </div>
            ...
            <div className="bg-white rounded-3xl p-8 lg:p-10 shadow-sm">
              {status === "success" ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="font-display text-3xl font-light mb-3" style={{ color: 'var(--forest)' }}>Message received.</h3>
                  <p className="text-slate-500 text-sm leading-relaxed">
                    Thank you for reaching out. We&apos;ll respond within 48 hours. Every connection plants a seed.
                  </p>
                  <button
                    onClick={() => setStatus("idle")}
                    className="mt-6 text-sm font-medium" style={{ color: 'var(--leaf)' }}
                  >
                ...
                <>
                  <h2 className="font-display text-3xl font-light mb-2" style={{ color: 'var(--forest)' }}>Get in touch</h2>
                  <p className="text-slate-400 text-sm mb-8">Whether you want to donate, partner, volunteer, or simply learn more — we want to hear from you.</p>

                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <label className="text-xs font-medium tracking-wide uppercase mb-1.5 block" style={{ color: 'var(--forest)' }}>Name</label>
                        <input
                          required
                          value={form.name}
                          ...
                          style={{ borderColor: 'rgba(74,140,82,0.2)', background: 'var(--mist)', '--tw-ring-color': 'rgba(74,140,82,0.3)'} as React.CSSProperties}
                        />
                      </div>
                      <div>
                    ...
                    <div>
                      <label className="text-xs font-medium tracking-wide uppercase mb-1.5 block" style={{ color: 'var(--forest)' }}>Subject</label>
                      <select
                        value={form.subject}
                        onChange={e => setForm({ ...form, subject: e.target.value })}
                        disabled={status === "submitting"}
                        className="w-full px-4 py-3 rounded-xl border text-sm outline-none transition-all disabled:opacity-50"
                        style={{ borderColor: 'rgba(74,140,82,0.2)', background: 'var(--mist)', color: form.subject ? 'var(--charcoal)' : '#94a3b8' }}
                      >
                        <option value="">Select a reason...</option>
                        <option>I want to donate</option>
                        <option>I want to sponsor a child</option>
                        <option>I want to partner / volunteer</option>
                    ...
                    <div>
                      <label className="text-xs font-medium tracking-wide uppercase mb-1.5 block" style={{ color: 'var(--forest)' }}>Message</label>
                      <textarea
                        required
                        rows={5}
                        value={form.message}
                    ...
                    {status === "error" && (
                      <div className="p-4 rounded-xl bg-red-50 text-red-700 text-xs font-medium flex items-center gap-2.5 border border-red-100">
                        ...
                        <span>Submission deployment encountered an error. Please verify network and retry.</span>
                      </div>
                    )}
